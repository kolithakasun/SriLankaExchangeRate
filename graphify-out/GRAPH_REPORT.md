...
## Corpus Check
- 69 files · ~27,598 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 495 nodes · 1139 edges · 17 communities (14 shown, 3 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 8 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9bc6b3f8`
...
- `handler` --calls--> `resolveForecastWindow()`  [EXTRACTED]
  netlify/functions/forecast.ts → shared/config/ranges.ts
- `handler` --calls--> `buildForecast()`  [EXTRACTED]
  netlify/functions/forecast.ts → shared/utils/forecast.ts
...
- `withLiveDailyHistory()` --calls--> `nowIso()`  [EXTRACTED]
  netlify/functions/history.ts → shared/utils/time.ts
- `loadStoredDaily()` --calls--> `dailyPointsToAggregates()`  [EXTRACTED]
  netlify/functions/lib/forecast-references.ts → shared/utils/forecast.ts
...
Cohesion: 0.08
Nodes (49): buildPrompt(), geminiNarration(), groqNarration(), narrateForecast(), NarrationSource, PROVIDERS, referencePromptBlock(), templateNarration() (+41 more)
...
### Community 4 - "functions/forecast.ts"
Cohesion: 0.09
Nodes (38): handler, handler, handler, getAvailableProviders(), withReferences(), checkRefreshAllowed(), getClientIp(), handleOptions() (+30 more)
...
## Knowledge Gaps
- **138 isolated node(s):** `$schema`, `typescript`, `oxc`, `react/rules-of-hooks`, `warn` (+133 more)
